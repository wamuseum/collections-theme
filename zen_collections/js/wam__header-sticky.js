    ...
    attach: function () {
      ...
      if (!$('html').hasClass('lt-ie9')) {
        ...
        var mainAboveHeight = $('#page-header').outerHeight(),
          mainNavHeight = $('#page-header nav').outerHeight(),
          nojumplist = 1,
          jumplistAboveHeight = 0,
          jumplistNavHeight = 0;

        // check to see if .jumplist exists
        if ($('.jumplist').length) {
          jumplistAboveHeight = $('.jumplist').offset();
          jumplistNavHeight = $('.jumplist').outerHeight();
